(() => {
  ...
  class ThemeButton extends HTMLElement {
    ...
    forceDark() {
      ...
      this._isMoved = true;
      this._container.classList.add('moved');
      if (this._components) {
        this._components.style.transform = 'translateX(100%)';
        // 直接触发点击动画
        this._components.onclick();
      }
    ...
    forceLight() {
      if (!this._container) return;
      this._isMoved = false;
      this._container.classList.remove('moved');
      if (this._components) {
        this._components.style.transform = 'translateX(0)';
        // 直接触发点击动画
        this._components.onclick();
      }
    ...
    updateButtonState() {
      ...
      this._components = container.querySelector('.components');
    ...
    connectedCallback() {
      ...
      this._isMoved = initialValue === 'dark';
